...
function enviarParaPlanilha() {
    ...
    const nulosBrancos = parseInt(document.getElementById("nulosBrancos").value);
    const partidos = [];
    let votosValidos = 0; // Inicializa votos válidos

    for (let i = 1; i <= 9; i++) {
        const nomePartido = document.getElementById(`partidoNome${i}`).value;
        const votos = parseInt(document.getElementById(`partido${i}`).value);
        const cor = document.getElementById(`partidoCor${i}`).value; // Cor opcional

        // Adicionar os dados do partido ao array
        partidos.push({
            Partido: nomePartido,
            Votos: votos,
            Cor: cor,
            VagasObtidas: 0, // Inicializa VagasObtidas, que será calculada a seguir
            Sobras: 0 // Inicializa Sobras, que será calculada a seguir
        });

        votosValidos += votos; // Soma os votos válidos
    }

    // Calcular Votos Totais
    const votosTotais = votosValidos + nulosBrancos;

    // Calcular Vagas Obtidas para cada partido
    ...
    if (sobras > 0) {
        // Criar um array com as proporções de votos válidos para a distribuição das sobras
        const votosPorVaga = partidos.map(partido => ({
            Partido: partido.Partido,
            Votos: partido.Votos - partido.VagasObtidas * (votosValidos / vagas)
        }));

        // Ordenar por maior quantidade de votos (mais sobras)
        votosPorVaga.sort((a, b) => b.Votos - a.Votos);

        // Distribuir as sobras
        for (let i = 0; i < sobras; i++) {
            // Distribui uma vaga para o partido com mais votos sobrando
            partidos.find(p => p.Partido === votosPorVaga[i % votosPorVaga.length].Partido).Sobras++;
        }
    }

    // Atualizar Vagas Obtidas com Sobras
    partidos.forEach(partido => {
        partido.VagasObtidas += partido.Sobras;
    });

    // Organizar dados para enviar ao Google Sheets
    const data = {
        data: [
            { Vagas: vagas, VotosNulosBrancos: nulosBrancos, VotosTotais: votosTotais, VotosValidos: votosValidos },
            ...partidos // Adicionar os dados de cada partido
        ]
    };

    // URL do endpoint do SheetDB (substitua pelo seu)
    const sheetdbUrl = 'https://sheetdb.io/api/v1/j2m36qig7c123';

    // Fazer a requisição POST para enviar os dados
    fetch(sheetdbUrl, {
        method: 'POST',
        headers: {
